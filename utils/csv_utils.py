"""CSV parsing and snapshot date extraction from filename (n8n Code node logic)."""
import csv
import io
import re
...
def snapshot_date_from_filename(file_name: str) -> str | None:
    """
    Extract DDMMYY pattern from filename → YYYY-MM-DD.
...
def parse_csv_rows(
    ...
    """
    Parse CSV text and attach snapshot date from filename to each row.
    """
    ...
    out = []
    for row in rows:
        # If no snapshot date from filename, look for snapshot_date column in first row
