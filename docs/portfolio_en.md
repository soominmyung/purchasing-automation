...
**AI analysis quality-evaluation (Evaluator) agent** A separate analysis evaluation agent cross-references the analysis agent's output (JSON) against the source supplier/item history it was based on, scoring it out of 10 on four criteria — **numeric accuracy, source faithfulness (hallucination check), soundness of reasoning, and sensitive-information leakage**. Because it compares against the exact history retrieved by the analysis agent — passed directly rather than re-fetched — it avoids the failure mode of misjudging a genuinely grounded statement as "unsourced" and wrongly scoring it as a hallucination.
...
**Hybrid design — separating deterministic computation from LLM reasoning** In the supplier-grouping stage, the recommended order date and quantity are **computed with a Python formula** based on `WksToOOS` (weeks to out-of-stock) and `CurrentStock`. Values a clear formula can produce are handled by deterministic code rather than left to LLM reasoning, removing any room for hallucination; the LLM then takes that result and focuses solely on contextual interpretation and document authoring over the prioritized, structured data. Keeping the calculation out of the LLM call also cuts token spend and latency.
