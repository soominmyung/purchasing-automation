...
- **명시적 메타데이터 검색 및 필터링** — 분석 에이전트가 히스토리를 조회할 때, LLM이 생성한 쿼리 텍스트에만 의존하지 않고 파이프라인이 이미 알고 있는 실제 공급업체명·아이템 코드를 메타데이터 필터로 함께 전달합니다. 검색 결과가 "의미적으로 비슷한 문서"가 아니라 "그 공급업체·품목에 실제로 해당하는 문서 중 의미적으로 가장 관련 있는 것"으로 정확히 좁혀집니다.
